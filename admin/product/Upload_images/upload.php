...
//$ID = $_REQUEST['ID'];
//echo "ID :".$ID;
$sql = "SELECT * FROM  product WHERE 1 order by proID Desc LIMIT 0,1";
$re = mysql_query($sql);
...
<title>Upload Image on DB</title><form action="admin.php?go=upload_img" method="post">
<div align="center">
...
if(isset($_POST['ok_num']))
{
	...
	echo "<hr  class='table'/>";
?>	
	<form action='admin.php?go=process&num=<?php echo $num?>&ID=<?php echo $ID?>' method='post' enctype='multipart/form-data'>
	...
	?>
          ...
          <tr>
            ...
            <td><input name='text<?php echo $i?>' type='text' class="text" size="50" width="70" /></td>
          </tr>
        </table>
		<hr class="table" />
		<?php }?>	
